...
SEED=1
MODEL_TYPE="nn.seq2seq.aether.Aether"
EXPERIMENT_EXT=""
...
TRAINING_ARGS='--rff_std 1.0 --vel_norm_norm --version_checkpoints 50 --add_uniform_prior --no_edge_prior 0.5 --batch_size 64 --lr 5e-4 --use_adam --num_epochs 600 --lr_decay_factor 0.5 --lr_decay_steps 600 --normalize_kl --normalize_nll --tune_on_mse --val_teacher_forcing --teacher_forcing_steps -1'
mkdir -p $WORKING_DIR
...
CUDA_VISIBLE_DEVICES=$GPU python -u experiments/electrostatic/main.py --gpu \
  --mode visualize_field --report_error_norm --load_best_model --test_pred_steps 20 \
  --data_path $DATA_PATH --working_dir $WORKING_DIR $MODEL_ARGS $TRAINING_ARGS
CUDA_VISIBLE_DEVICES=$GPU python -u experiments/electrostatic/main.py --gpu \
