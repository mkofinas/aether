...
class AetherDynamicVars(nn.Module):
...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.edge_filter = AnisotropicEdgeFilter(
            self.num_relative_features+inp_size+self.num_dims, self.num_pos_features,
    ...
    def compute_feat_transform(self, inputs, node_masks, predicted_field):
        ...
        for inputs, node_masks in zip(inp_list, node_masks_list):
            ...
            edge_attr = create_augmented_edge_attr_pos_vel(
                flat_inp, send_edges, recv_edges, batched=True)
            edge_pos = edge_attr[..., self.edge_pos_idx]
            edge_attr = torch.cat([edge_attr, rel_feat[recv_edges]], -1)
            edge_attr = self.edge_filter(edge_attr, edge_pos)
            x_skip = edge_attr
            result_shape = (tmp_batch, edge_attr.size(-1))
            x = self.batch_edge2node(edge_attr, result_shape, recv_edges) + self.mlp1(rel_feat)
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.edge_filter = nn.ModuleList(
            [AnisotropicEdgeFilter(
                self.num_relative_features+input_size+self.num_dims, self.num_pos_features,
    ...
    def forward(self, inputs, hidden, edges, node_masks, graph_info,
        ...
        if num_vars > 1:
            ...
            edge_attr = create_augmented_edge_attr_pos_vel(extended_inputs, send_edges, recv_edges)
            edge_pos = edge_attr[..., self.edge_pos_idx]
            edge_attr = torch.cat([edge_attr, rel_feat[:, recv_edges]], -1)
        ...
        inp_r = self.input_r(rel_feat).view(current_inputs.size(0), num_vars, -1)
        inp_i = self.input_i(rel_feat).view(current_inputs.size(0), num_vars, -1)
        inp_n = self.input_n(rel_feat).view(current_inputs.size(0), num_vars, -1)
