...
from nn.nn.anisotropic_filter import AnisotropicEdgeFilter
from nn.nn.fourier_feature_mapper import FourierFeatureMapper
from nn.utils.canonicalization import canonicalize_augmented_inputs, create_augmented_edge_attr_pos_vel
from nn.utils.local_to_global import Globalizer


class AetherOriginDynamicVars(nn.Module):
    def __init__(self, params):
        ...
        hidden_size = params['field_hidden']
        ...
        self.coordinate_embedding = FourierFeatureMapper(
            2, hidden_size // 2, std=rff_std)
        self.angular_embedding = nn.Linear(2, hidden_size)

    def predict_field(self, x, masks=None):
        if masks is None:
            masks = torch.ones_like(x[..., 0], dtype=torch.bool)
        predicted_field = torch.zeros_like(x[..., :2])

        coords = torch.cat([
            x[masks.bool()][..., :2],
            F.normalize(x[masks.bool()][..., 2:], dim=-1)], -1)

        rff_positions = self.coordinate_embedding(coords[..., :2])
        angles_emb = self.angular_embedding(coords[..., 2:])
        field_inputs = torch.cat([rff_positions, angles_emb], -1)

        predicted_field_flat = self.field_net(field_inputs)
        predicted_field[masks.bool()] = predicted_field_flat
        return predicted_field, coords

    @staticmethod
    def _make_grid(linspaces):
        test_positions = torch.stack(torch.meshgrid(*linspaces), dim=-1).flatten(0, -2)
        return test_positions

    def field_at_range(self, field_min, field_max):
        grid_size = 101
        linspaces = [
            torch.linspace(field_min[0], field_max[0], grid_size),
            torch.linspace(field_min[1], field_max[1], grid_size),
        ]
        test_positions = self._make_grid(linspaces)
        test_velocities = torch.zeros_like(test_positions)
        test_velocities[..., 0] = 1.0
        test_positions = torch.cat([test_positions, test_velocities], -1)

        device = next(self.parameters()).device
        predicted_field, _ = self.predict_field(test_positions.to(device))
        predicted_field = predicted_field.detach().cpu()
        return predicted_field, test_positions

    def field_at_se2_range(self, field_min, field_max, grid_size=101,
                           angle_grid_size=36):
        linspaces = [
            torch.linspace(field_min[0], field_max[0], grid_size),
            torch.linspace(field_min[1], field_max[1], grid_size),
            torch.from_numpy(np.linspace(0.0, 2.0 * np.pi, angle_grid_size, endpoint=False)).float(),
        ]
        test_positions = self._make_grid(linspaces)
        test_positions = torch.cat([
            test_positions[..., :2],
            test_positions[..., [2]].cos(),
            test_positions[..., [2]].sin()
        ], -1)

        device = next(self.parameters()).device
        predicted_field, _ = self.predict_field(test_positions.to(device))
        predicted_field = predicted_field.reshape(grid_size, grid_size, angle_grid_size, -1)
        test_positions = test_positions.reshape(grid_size, grid_size, angle_grid_size, -1)
        return predicted_field, test_positions
    ...
    def single_step_forward(self, inputs, node_masks, graph_info, decoder_hidden,
                            edge_logits, hard_sample, current_field):
        ...
        predictions, decoder_hidden = self.decoder(
            inputs, decoder_hidden, edges, node_masks, graph_info, current_field)
        return predictions, decoder_hidden, edges

    def normalize_inputs(self, inputs, node_masks):
        predicted_field, _ = self.predict_field(inputs, node_masks)
        return self.encoder.normalize_inputs(inputs, node_masks, predicted_field)
    ...
    def calculate_loss(self, inputs, node_masks, node_inds, graph_info, is_train=False, teacher_forcing=True, return_edges=False, return_logits=False, use_prior_logits=False, normalized_inputs=None):
        ...
        x = inputs[:, :-1].contiguous()
        predicted_field, _ = self.predict_field(x, node_masks[:, :-1])

        prior_logits, posterior_logits, _ = self.encoder(
            inputs[:, :-1], node_masks[:, :-1], node_inds, graph_info,
            normalized_inputs, predicted_field)
        ...
        for step in range(num_time_steps-1):
            if (teacher_forcing and (teacher_forcing_steps == -1 or step < teacher_forcing_steps)) or step == 0:
                ...
                current_field, _ = self.predict_field(predictions, node_masks[:, step])
            ...
            predictions, decoder_hidden, edges = self.single_step_forward(
                current_inputs, current_node_masks, current_graph_info,
                decoder_hidden, current_p_logits, hard_sample, current_field)
    ...
    def predict_future(self, inputs, masks, node_inds, graph_info, burn_in_masks):
        ...
        * burn_in_masks is set to 1 whenever we're supposed to feed in that variable's state
        ...
        for step in range(total_timesteps-1):
            ...
            current_field, _ = self.predict_field(encoder_inp, current_masks)
            current_edge_logits, prior_hidden = self.encoder.single_step_forward(
                encoder_inp, current_masks, current_node_inds,
                current_graph_info, prior_hidden, current_field)
            predictions, decoder_hidden, _ = self.single_step_forward(
                encoder_inp, current_masks, current_graph_info, decoder_hidden,
                current_edge_logits, True, current_field)
...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        if pos_representation not in ('cart', 'polar'):
            raise ValueError
        self.edge_pos_idx = [0, 1, 2] if pos_representation == 'cart' else [2, 3, 4]
        self.num_dims = 3 if self.use_3d else 2
        ...
        self.edge_filter = AnisotropicEdgeFilter(
            2*self.num_relative_features+inp_size+self.num_dims, self.num_pos_features,
            hidden_size, hidden_size, hidden_size, do_prob=dropout,
            bn=not no_bn, act='relu')
        self.edge_filter_out_size = hidden_size

        self.mlp1 = RefNRIMLP(inp_size+self.num_dims+self.num_relative_features, hidden_size, hidden_size, dropout, no_bn=no_bn)
    ...
    def normalize_inputs(self, inputs, node_masks, predicted_field):
        if self.normalize_mode == 'normalize_inp':
            raise NotImplementedError
        elif self.normalize_mode == 'normalize_all':
            result = self.compute_feat_transform(inputs, node_masks, predicted_field)
        return result

    def compute_feat_transform(self, inputs, node_masks, predicted_field):
        ...
        for inputs, node_masks in zip(inp_list, node_masks_list):
            ...
            rel_feat, _ = canonicalize_augmented_inputs(flat_inp)

            num_objects = flat_inp.size(0)
            flat_inp = torch.cat([flat_inp, self._origin.unsqueeze(0).cuda()], 0)
            _send_edges = torch.cat([send_edges, torch.full((num_objects,), num_objects).cuda().long()])
            _recv_edges = torch.cat([recv_edges, torch.arange(num_objects).cuda().long()])

            edge_attr = create_augmented_edge_attr_pos_vel(
                flat_inp, _send_edges, _recv_edges, batched=True)
            origin_edge_attr = edge_attr[-num_objects:]
            edge_attr = edge_attr[:-num_objects]

            edge_pos = edge_attr[..., self.edge_pos_idx]
            edge_attr = torch.cat([edge_attr, rel_feat[recv_edges], origin_edge_attr[recv_edges]], -1)
            edge_attr = self.edge_filter(edge_attr, edge_pos)
            x_skip = edge_attr
            result_shape = (tmp_batch, edge_attr.size(-1))
            x = self.batch_edge2node(edge_attr, result_shape, recv_edges) + self.mlp1(torch.cat([rel_feat, origin_edge_attr], -1))
    ...
    def forward(self, inputs, node_masks, all_node_inds, all_graph_info,
                normalized_inputs=None, predicted_field=None):
        if inputs.size(0) > 1:
        ...
        if self.normalize_mode == 'normalize_all':
            if normalized_inputs is not None:
                ...
                x = torch.cat(self.normalize_inputs(inputs, node_masks,
                                                    predicted_field), dim=0)
        else:
        ...
        for timestep in range(num_timesteps-1, -1, -1):
            ...
            if len(node_inds) <= 1:
                # all_reverse_states.append(torch.empty(1, 0, self.rnn_hidden_size, device=inputs.device))
                continue
            send_edges, recv_edges, _ = all_graph_info[0][timestep]
            send_edges, recv_edges = send_edges.cuda(non_blocking=True), recv_edges.cuda(non_blocking=True)
            global_send_edges = node_inds[send_edges]
    ...
    def single_step_forward(self, inputs, node_masks, node_inds, all_graph_info,
                            forward_state, predicted_field):
        if self.normalize_mode == 'normalize_all':
            x = self.normalize_inputs(inputs, node_masks, predicted_field)[0]
        else:
            raise NotImplementedError
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.edge_filter = nn.ModuleList(
            [AnisotropicEdgeFilter(
                2*self.num_relative_features+input_size+self.num_dims, self.num_pos_features,
                n_hid, n_hid, n_hid, do_prob=do_prob, bn=False, act='relu')
            for _ in range(edge_types)]
            )
        self.edge_filter_out_size = n_hid

        self.input_r = nn.Linear(self.num_relative_features+input_size+self.num_dims, n_hid, bias=True)
        self.input_i = nn.Linear(self.num_relative_features+input_size+self.num_dims, n_hid, bias=True)
        self.input_n = nn.Linear(self.num_relative_features+input_size+self.num_dims, n_hid, bias=True)
        ...
        self.globalizer = Globalizer(num_dims=2)
    ...
    def forward(self, inputs, hidden, edges, node_masks, graph_info,
        ...
        current_extended_inputs = extended_inputs[:, node_inds]
        num_vars = current_hidden.size(1)

        rel_feat, Rinv = canonicalize_augmented_inputs(current_extended_inputs)
        ...
        if num_vars > 1:
            ...
            num_objects = extended_inputs.size(1)
            full_indices = torch.unique(torch.cat([recv_edges, send_edges]))
            num_origin_edges = full_indices.shape[0]
            extended_inputs = torch.cat([extended_inputs, self._origin.expand_as(extended_inputs[:, [0]]).cuda()], 1)
            _send_edges = torch.cat([send_edges, torch.full((num_origin_edges,), num_objects).cuda().long()])
            _recv_edges = torch.cat([recv_edges, full_indices])

            edge_attr = create_augmented_edge_attr_pos_vel(extended_inputs, _send_edges, _recv_edges)
            origin_edge_attr = edge_attr[:, -num_origin_edges:]
            edge_attr = edge_attr[:, :-num_origin_edges]
            ...
            present_incoming = present_msgs[:, edge2node_inds[:, 0], :].clone()
            for i in range(1, edge2node_inds.size(1)):
                present_incoming += present_msgs[:, edge2node_inds[:, i], :]
            present_agg_msgs = present_incoming/(num_vars-1)
        elif num_vars == 0:
        ...
        inp_r = self.input_r(torch.cat([rel_feat, origin_edge_attr], -1)).view(current_inputs.size(0), num_vars, -1)
        inp_i = self.input_i(torch.cat([rel_feat, origin_edge_attr], -1)).view(current_inputs.size(0), num_vars, -1)
        inp_n = self.input_n(torch.cat([rel_feat, origin_edge_attr], -1)).view(current_inputs.size(0), num_vars, -1)
        r = torch.sigmoid(inp_r + self.hidden_r(agg_msgs) + self.present_r(present_agg_msgs))
        i = torch.sigmoid(inp_i + self.hidden_i(agg_msgs) + self.present_i(present_agg_msgs))
        n = torch.tanh(inp_n + r*self.hidden_h(agg_msgs) + self.present_n(present_agg_msgs))
