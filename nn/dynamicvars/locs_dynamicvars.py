...
class LoCSDynamicVars(nn.Module):
...
class Encoder(nn.Module):
    ...
    def compute_feat_transform(self, inputs, node_masks):
        ...
        for inputs, node_masks in zip(inp_list, node_masks_list):
            ...
            edge_attr = create_edge_attr_pos_vel(flat_inp, send_edges, recv_edges, batched=True)
...
class Decoder(nn.Module):
        ...
        self.edge_filter = nn.ModuleList([
            nn.Sequential(
                AnisotropicEdgeFilter(
                    self.num_relative_features+input_size,
                    self.num_pos_features, n_hid, n_hid, n_hid,
                    do_prob=do_prob, bn=False, act='relu'),
                nn.ReLU(),
            )
    ...
    def forward(self, inputs, hidden, edges, node_masks, graph_info):
        ...
        rel_feat, Rinv = canonicalize_inputs(current_inputs)
        ...
        if num_vars > 1:
            ...
            edge_attr = create_edge_attr_pos_vel(inputs, send_edges, recv_edges)
            edge_pos = edge_attr[..., self.edge_pos_idx]
            edge_attr = torch.cat([edge_attr, rel_feat[:, recv_edges]], -1)
