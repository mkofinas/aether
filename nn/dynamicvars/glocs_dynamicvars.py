...
from nn.utils.canonicalization import canonicalize_inputs, create_edge_attr_pos_vel
from nn.utils.local_to_global import Globalizer


class GLoCSDynamicVars(nn.Module):
    ...
    def single_step_forward(self, inputs, node_masks, graph_info, decoder_hidden,
        ...
        predictions, decoder_hidden = self.decoder(
            inputs, decoder_hidden, edges, node_masks, graph_info)
    ...
    def calculate_loss(self, inputs, node_masks, node_inds, graph_info, is_train=False, teacher_forcing=True, return_edges=False, return_logits=False, use_prior_logits=False, normalized_inputs=None):
        ...
        for step in range(num_time_steps-1):
            ...
            predictions, decoder_hidden, edges = self.single_step_forward(
                ...
                decoder_hidden, current_p_logits, hard_sample)
...
class Encoder(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.edge_filter = AnisotropicEdgeFilter(
            2*self.num_relative_features+inp_size, self.num_pos_features,
            hidden_size, hidden_size, hidden_size, do_prob=dropout,
            bn=not no_bn, act='relu')
        self.edge_filter_out_size = hidden_size

        self.mlp1 = RefNRIMLP(inp_size+self.num_relative_features, hidden_size, hidden_size, dropout, no_bn=no_bn)
    ...
    def compute_feat_transform(self, inputs, node_masks):
        ...
        for inputs, node_masks in zip(inp_list, node_masks_list):
            ...
            rel_feat, _ = canonicalize_inputs(flat_inp)
            ...
            edge_attr = create_edge_attr_pos_vel(
...
class Decoder(nn.Module):
    def __init__(self, params):
        ...
        self.edge_filter = nn.ModuleList(
            [AnisotropicEdgeFilter(
                2*self.num_relative_features+input_size, self.num_pos_features,
                n_hid, n_hid, n_hid, do_prob=do_prob, bn=False, act='relu')
            for _ in range(edge_types)]
            )
        self.edge_filter_out_size = n_hid

        self.input_r = nn.Linear(self.num_relative_features+input_size, n_hid, bias=True)
        self.input_i = nn.Linear(self.num_relative_features+input_size, n_hid, bias=True)
        self.input_n = nn.Linear(self.num_relative_features+input_size, n_hid, bias=True)
    ...
    def forward(self, inputs, hidden, edges, node_masks, graph_info):
        ...
        rel_feat, Rinv = canonicalize_inputs(current_extended_inputs)
        ...
        if num_vars > 1:
            ...
            edge_attr = create_edge_attr_pos_vel(extended_inputs, _send_edges, _recv_edges)
