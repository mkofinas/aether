...
class AetherCharges(nn.Module):
    def __init__(self, params):
        ...
        self.field_net = nn.Sequential(
            nn.Linear(hidden_size+charge_embedding_dim, hidden_size),
            nn.SiLU(),
    ...
    def calculate_loss(
        ...
        charge_emb = self.charge_embedding(self.charge_to_index(charges[:, :num_objects]))
        predicted_field, _ = self.predict_field(x, charge_emb)
        ...
        for step in range(num_time_steps-1):
            if (teacher_forcing and (teacher_forcing_steps == -1 or step < teacher_forcing_steps)) or step == 0:
                ...
                current_field, _ = self.predict_field(predictions, charge_emb)
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, predicted_field, charge_emb):
        ...
        edge_attr = torch.cat([edge_attr, _charge_emb[:, self.recv_edges], _charge_emb[:, self.send_edges]], -1)

        # Anisotropic filter generation
        edge_attr = self.edge_filter(edge_attr, edge_pos)

        edge_attr_prev = edge_attr
        res_x = self.res1(torch.cat([rel_feat, _charge_emb], -1))
