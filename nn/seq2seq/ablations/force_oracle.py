...
class ForceOracle(nn.Module):
    """Insert global origin as virtual node and include relative information to
    it in nodes states, append gt forces in node states
    """
    ...
    def calculate_loss(
            self, inputs, is_train=False, teacher_forcing=True,
...
class Encoder(nn.Module):
    def __init__(self, params):
        ...
        self.res1 = nn.Linear(inp_size+self.num_relative_features+self.num_dims, hidden_size)
    ...
    def forward(self, inputs, charges=None):
        ...
        # Field effect
        external_field = self.field(x.reshape(-1, 2 * self.num_dims)[..., :self.num_dims])
        external_force = external_field.reshape(*x.shape[:-1], self.num_dims) * charges[:, :self.num_vars, None, None]
        extended_inputs = torch.cat([x, external_force], -1)
    ...
    def single_step_forward(self, inputs, prior_state, charges=None):
        # Field effect
        external_field = self.field(inputs.reshape(-1, 2 * self.num_dims)[..., :self.num_dims])
        external_force = external_field.reshape(*inputs.shape[:-1], self.num_dims) * charges[:, :self.num_vars, None]
        extended_inputs = torch.cat([inputs, external_force], -1)
...
class MarkovDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.res1 = nn.Linear(in_size+self.num_relative_features+self.num_dims, msg_hid)

        self.edge_filter = MLPEdgeFilter(
            2*self.num_relative_features+in_size+self.num_dims, self.num_pos_features,
    ...
    def forward(self, inputs, hidden, edges, charges):
        ...
        # [batch_size, num_objects, num_dims]
        start_idx = 1 if self.skip_first_edge_type else 0

        # Field effect
        external_field = self.field(inputs.reshape(-1, 2 * self.num_dims)[..., :self.num_dims])
        external_force = external_field.reshape(*inputs.shape[:2], self.num_dims) * charges[:, :self.num_vars, None].cuda()
        extended_inputs = torch.cat([inputs, external_force], -1)
        ...
        return inputs + pred + output_effect, None
...
class RecurrentDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.present_msg_fc1 = nn.ModuleList(
            [nn.Linear(2*self.num_relative_features+input_size+self.num_dims, n_hid) for _ in range(edge_types)]
        )
        ...
        self.input_r = nn.Linear(input_size+self.num_relative_features+self.num_dims, n_hid, bias=True)
        self.input_i = nn.Linear(input_size+self.num_relative_features+self.num_dims, n_hid, bias=True)
        self.input_n = nn.Linear(input_size+self.num_relative_features+self.num_dims, n_hid, bias=True)
