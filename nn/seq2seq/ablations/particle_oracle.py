...
from nn.utils.oracle_global_to_local import OracleLocalizer
from nn.utils.local_to_global import Globalizer


class AetherParticleOracle(nn.Module):
    """Explicitly use field particle locations as extra nodes. Use separate network"""
    ...
    def predict_future(self, inputs, prediction_steps, return_edges=False,
        ...
        for step in range(prediction_steps):
            current_edge_logits, prior_hidden = self.encoder.single_step_forward(
                predictions, prior_hidden, charges=charges)
            predictions, decoder_hidden, edges = self.single_step_forward(
...
class Encoder(nn.Module):
    def __init__(self, params):
        ...
        self._send_edges = self.send_edges
        self._recv_edges = self.recv_edges
        self.use_3d = params.get('use_3d', False)
        ...
        self.field_filter = MLPEdgeFilter(
            self.num_relative_features+inp_size, self.num_pos_features,
            hidden_size, hidden_size, hidden_size, do_prob=dropout)

        extra_edges = torch.cartesian_prod(
            torch.arange(num_vars, num_vars + self.field.positions.size(0)),
            torch.arange(num_vars)).T.numpy()
        self._send_edges = np.concatenate([self._send_edges, extra_edges[0]])
        self._recv_edges = np.concatenate([self._recv_edges, extra_edges[1]])

        self.localizer = OracleLocalizer(
            self.num_vars, self.field.positions.size(0), use_3d=self.use_3d,
            pos_representation='polar',
        )
    ...
    def field_edge2node(self, field_edge_attr):
        field_agg_msgs = scatter(
            field_edge_attr,
            torch.from_numpy(self._recv_edges[self.recv_edges.shape[0]:]).to(field_edge_attr.device), dim=1, reduce='mean')
        field_agg_msgs = field_agg_msgs.contiguous()
        return field_agg_msgs

    def forward(self, inputs, charges=None):
        ...
        external_field = self.field._normalize(self.field.positions)
        external_field = torch.cat(
            [external_field, torch.zeros_like(external_field)], -1)
        extended_inputs = torch.cat(
            [x, external_field.unsqueeze(0).unsqueeze(2).repeat(x.shape[0], 1, x.shape[2], 1).to(x.device)], 1)

        rel_feat, Rinv, edge_attr, edge_pos, field_edge_attr, field_edge_pos = self.localizer(
            x, extended_inputs)

        # Anisotropic filter generation
        edge_attr = self.edge_filter(edge_attr, edge_pos)
        field_edge_attr = self.field_filter(field_edge_attr, field_edge_pos)

        edge_attr_prev = edge_attr
        res_x = self.res1(rel_feat)
        x = self.edge2node(edge_attr) + res_x + self.field_edge2node(field_edge_attr)
        x = self.mlp3(x)
        x = self.node2edge(x)
    ...
    def single_step_forward(self, inputs, past_inputs, prior_state, charges=None):
        # Inputs is shape [batch, num_vars, input_size]
        external_field = self.field._normalize(self.field.positions)
        external_field = torch.cat(
            [external_field, torch.zeros_like(external_field)], -1)
        extended_inputs = torch.cat(
            [inputs, external_field.repeat(inputs.shape[0], 1, 1).to(inputs.device)], 1)
...
class MarkovDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.field_filter = MLPEdgeFilter(
            self.num_relative_features+in_size, self.num_pos_features,
            msg_hid, msg_hid, msg_out,
            do_prob=self.dropout_prob)

        self.field = params['field']

        extra_edges = torch.cartesian_prod(
            torch.arange(self.num_vars, self.num_vars + self.field.positions.size(0)),
            torch.arange(self.num_vars)).T.numpy()
        self._send_edges = np.concatenate([self._send_edges, extra_edges[0]])
        self._recv_edges = np.concatenate([self._recv_edges, extra_edges[1]])
    ...
    def forward(self, inputs, hidden, edges, charges):
        ...
        external_field = torch.cat(
            [external_field, torch.zeros_like(external_field)], -1)
        extended_inputs = torch.cat(
            [inputs, external_field.repeat(inputs.shape[0], 1, 1)], 1)
        ...
        field_agg_msgs = scatter(
            field_edge_attr,
            torch.from_numpy(self._recv_edges[self.recv_edges.shape[0]:]).to(all_msgs.device), dim=1, reduce='mean')
        field_agg_msgs = field_agg_msgs.contiguous()

        # Skip connection
        res_x = self.res1(rel_feat)
        aug_inputs = agg_msgs + res_x + field_agg_msgs
...
class RecurrentDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.field_r = nn.Linear(n_hid, n_hid, bias=True)
        self.field_i = nn.Linear(n_hid, n_hid, bias=True)
        self.field_n = nn.Linear(n_hid, n_hid, bias=True)
        ...
        self.num_used_edge_types = (
            edge_types - 1 if self.skip_first_edge_type else edge_types
        )
        self.edge_filter = MLPEdgeFilter(
            self.num_relative_features+input_size, self.num_pos_features,
            n_hid, n_hid, n_hid * self.num_used_edge_types,
            do_prob=self.dropout_prob)
        ...
        extra_edges = torch.cartesian_prod(
            torch.arange(num_vars, num_vars + self.field.positions.size(0)),
            torch.arange(num_vars)).T
        self._send_edges = torch.cat([self._send_edges, extra_edges[0]])
        self._recv_edges = torch.cat([self._recv_edges, extra_edges[1]])
    ...
    def forward(self, inputs, hidden, edges, charges):
        ...
        present_all_msgs = torch.sum(
            edge_attr.view(*edge_attr.shape[:-1], -1, self.num_used_edge_types)
            * edges[..., start_idx:].unsqueeze(-2), -1)

        # Aggregate all msgs to receiver
        present_agg_msgs = scatter(present_all_msgs, self.recv_edges.to(all_msgs.device), dim=1, reduce='mean')
        present_agg_msgs = present_agg_msgs.contiguous()

        present_field_agg_msgs = scatter(
            field_edge_attr,
            self._recv_edges[self.recv_edges.shape[0]:].to(all_msgs.device), dim=1, reduce='mean')
        present_field_agg_msgs = present_field_agg_msgs.contiguous()

        # GRU-style gated aggregation
        inp_r = self.input_r(rel_feat) + self.field_r(present_field_agg_msgs) + self.present_r(present_agg_msgs)
        inp_i = self.input_i(rel_feat) + self.field_i(present_field_agg_msgs) + self.present_i(present_agg_msgs)
        inp_n = self.input_n(rel_feat) + self.field_n(present_field_agg_msgs) + self.present_n(present_agg_msgs)
