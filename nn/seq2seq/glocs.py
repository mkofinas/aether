...
from nn.utils.origin_global_to_local import OriginLocalizer
from nn.utils.local_to_global import Globalizer


class GLoCS(nn.Module):
    ...
    def calculate_loss(
            ...
            return_logits=False, use_prior_logits=False):
...
class Encoder(nn.Module):
    def __init__(self, params):
        ...
        self.res1 = nn.Linear(inp_size+self.num_relative_features, hidden_size)
        ...
        self.localizer = OriginLocalizer(
            num_vars, use_3d=self.use_3d, pos_representation=params['pos_representation'])
...
class MarkovDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.res1 = nn.Linear(in_size+self.num_relative_features, msg_hid)

        self.edge_filter = MLPEdgeFilter(
            2*self.num_relative_features+in_size, self.num_pos_features,
            msg_hid, msg_hid, msg_out * self.num_used_edge_types,
            do_prob=self.dropout_prob)

        self.localizer = OriginLocalizer(self.num_vars, use_3d=self.use_3d,
...
class RecurrentDecoder(nn.Module):
    def __init__(self, params):
        ...
        self.present_msg_fc1 = nn.ModuleList(
            [nn.Linear(2*self.num_relative_features+input_size, n_hid) for _ in range(edge_types)]
        )
        ...
        self.input_r = nn.Linear(input_size+self.num_relative_features, n_hid, bias=True)
        self.input_i = nn.Linear(input_size+self.num_relative_features, n_hid, bias=True)
        self.input_n = nn.Linear(input_size+self.num_relative_features, n_hid, bias=True)
