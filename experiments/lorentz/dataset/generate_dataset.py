...
from experiments.lorentz.dataset.synthetic_sim import ChargedParticlesSim, DynamicSim, GravitySim, SpringSim, FixCharge
"""
...
def generate_dataset(num_sims, length, sample_freq, multiprocess=False, num_workers=10):
    ...
    if multiprocess:
        ...
        for i in range(num_sims):
            pool.apply_async(sim.sample_trajectory, (np.random.choice(list(range(1000000))), length, sample_freq), callback=collect)
        pool.close()
        pool.join()
    else:
