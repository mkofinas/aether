...
from experiments.lorentz.dataset4newton import NBodyDataset
from nn.state2state.gnn.gnn import GNN, RF_vel
from nn.state2state.egnn.egnn import EGNN, EGNN_vel
from nn.state2state.egnn_aether import EGNN_vel_Aether
from nn.state2state.locs.locs import LoCS
from nn.state2state.aether import Aether, ParallelAether
from nn.state2state.dynamic_field_aether import DynamicFieldAether
from nn.state2state.clof.clof import ClofNet, ClofNet_vel, ClofNet_vel_gbf
...
parser.add_argument('--nf', type=int, default=64, metavar='N',
                    help='learning rate')
parser.add_argument('--model', type=str, default='aether', metavar='N',
                    help='available models: gnn, baseline, linear, linear_vel, se3_transformer, egnn_vel, rf_vel, tfn, locs, aether, parallel_aether, film_aether, egnn_aether')
...
test_time_exp_dic = {'time': 0, 'counter': 0}
...
def main():
    ...
    elif args.model == 'aether':
        model = Aether(input_size=6, hidden_size=args.nf, dropout_prob=0.0, num_dims=3)
    elif args.model == 'parallel_aether':
        model = ParallelAether(input_size=6, hidden_size=args.nf, dropout_prob=0.0, num_dims=3)
    elif args.model == 'egnn_aether':
        model = EGNN_vel_Aether(in_node_nf=1, in_edge_nf=8, hidden_nf=args.nf, num_dims=3, device=device, n_layers=args.n_layers, recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
    elif args.model == 'dynamic_field_aether':
        model = DynamicFieldAether(input_size=6, hidden_size=args.nf, dropout_prob=0.0, num_dims=3)
...
def train(model, optimizer, epoch, loader, backprop=True):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        data = [d.flatten(0, 1) for d in data]
        ...
        if args.model == 'gnn':
            ...
            edge_attr = torch.cat([edge_attr, loc_dist], 1).detach()  # concatenate all edge properties
            loc_pred = model(nodes, loc.detach(), edges, vel, edge_attr)
        elif args.model in ('aether', 'parallel_aether'):
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=1)).unsqueeze(1).detach()
            rows, cols = edges
            loc_dist = torch.sqrt(torch.sum((loc[rows] - loc[cols])**2, 1)).unsqueeze(1)  # relative distances among locations
            edge_attr = torch.cat([edge_attr, loc_dist], 1).detach()  # concatenate all edge properties
            loc_pred = model(nodes, loc.detach(), edges, vel, edge_attr, charges)
        elif args.model == 'film_aether':
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=1)).unsqueeze(1).detach()
            rows, cols = edges
            loc_dist = torch.sqrt(torch.sum((loc[rows] - loc[cols])**2, 1)).unsqueeze(1)  # relative distances among locations
            edge_attr = torch.cat([edge_attr, loc_dist], 1).detach()  # concatenate all edge properties
            loc_pred = model(nodes, loc.detach(), edges, vel, edge_attr, charges, n_nodes)
        elif args.model == 'egnn_aether':
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=1)).unsqueeze(1).detach()
            rows, cols = edges
            loc_dist = torch.sum((loc[rows] - loc[cols])**2, 1).unsqueeze(1)  # relative distances among locations
        ...
        if args.time_exp:
            ...
            if backprop:
                ...
                test_time_exp_dic['time'] += t2 - t1
                test_time_exp_dic['counter'] += 1
                print("Test Forward average time: %.6f" % (test_time_exp_dic['time'] / test_time_exp_dic['counter']))
                logging.info("Test Forward average time: %.6f" % (test_time_exp_dic['time'] / test_time_exp_dic['counter']))
