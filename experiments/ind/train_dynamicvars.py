...
def train(model, train_data, val_data, params, train_writer, val_writer):
    ...
    tune_on_mse = params.get('tune_on_mse', True)
    ...
    for epoch in range(start_epoch, num_epochs+1):
                ...
                for sample in range(num_decoder_samples):
                    if normalize_inputs:
                        if masks is not None:
                            ...
                            normalized_inputs = model.normalize_inputs(inputs[:, :-1], None)
                    ...
                    if masks is not None:
                        ...
                        skip_node_inds = torch.zeros(sub_masks.shape[1]).long().to(sub_masks.device)
                        time_idx, entity_counts = sub_masks.nonzero()[:, -2].unique(return_counts=True)
                        skip_node_inds[time_idx] = entity_counts
                        if (skip_node_inds > 1).all():
                        # if (sub_masks.sum(1) >= 2).any() or not (skip_node_inds <= 1).any():
            ...
            for batch_ind, batch in enumerate(val_data_loader):
                ...
                if gpu:
                    ...
                    if masks is not None:
                        masks = masks.cuda(non_blocking=True)
                if masks is not None:
                    skip_node_inds = torch.zeros(masks.shape[1]).long().to(masks.device)
                    time_idx, entity_counts = masks.nonzero()[:, -2].unique(return_counts=True)
                    skip_node_inds[time_idx] = entity_counts
                    if (skip_node_inds > 1).all():
                    # if (sub_masks.sum(1) >= 2).any() or not (skip_node_inds <= 1).any():
        ...
        with torch.no_grad():
            val_mse, _, _, _ = eval_forward_prediction_dynamicvars_unnormalized(
        ...
        print("\tCURRENT VAL MSE: %f" % mean_mse)
